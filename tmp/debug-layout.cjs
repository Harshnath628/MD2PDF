...
await page.evaluate(()=>{window.print=()=>{}}); await page.locator('#print').click(); await page.emulateMedia({media:'print'});console.log('LAYOUT',await page.evaluate(()=>[...document.querySelectorAll('body>*,main,.preview,.paper-scroll,#document')].map(e=>({tag:e.tagName,cls:e.className?.baseVal??e.className,rect:{y:e.getBoundingClientRect().y,h:e.getBoundingClientRect().height},style:{height:getComputedStyle(e).height,margin:getComputedStyle(e).margin,padding:getComputedStyle(e).padding,overflow:getComputedStyle(e).overflow}})))); await page.pdf({path:'outputs/export-comparison/capabilities.pdf',printBackground:true,preferCSSPageSize:true}); console.log('DOWNLOAD',download.suggestedFilename());
